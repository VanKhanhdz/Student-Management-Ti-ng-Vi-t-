    ...
    def showStudent(self, listStudent):
        ...
        if (listStudent.__len__() > 0):
            for st in listStudent:
                print("{:<8} {:<18} {:<8}  {:<18}  {:<8} {:<8} {:<8} {:<8} {:<8}"
                    .format(str(st._id), str(st._fullname), str(st._sex), str(st._birthdate), str(st._scoremath), str(st._scoreliterature), 
                            str(st._scoreenglish),str(st._scoreavg),st._rankscore))
        print("\n")
